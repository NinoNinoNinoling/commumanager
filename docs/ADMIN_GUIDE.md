...
💡 **사용 시나리오**: 유저가 장기 휴식을 가거나, 개인 사정으로 일정 기간 활동이 불가능할 경우 관리자가 등록합니다.

#### 휴식 목록 조회 및 관리

테이블에서 각 휴식의 상세 정보를 확인하고 관리할 수 있습니다:

**필터 옵션:**
- **사용자 검색**: 사용자명으로 검색
- **진행 중만 표시**: 현재 휴식 중인 유저만 필터링

**테이블 컬럼:**
- **사용자**: 휴식 중인 유저 (@마스토돈 아이디)
- **시작일**: 휴식 시작 날짜
- **종료일**: 휴식 종료 예정 날짜
- **기간**: 총 N일
- **사유**: 휴식 사유 (개인 사정, 여행 등)
...
1. 테이블에서 해당 유저 행의 **"강제 해제"** 버튼 클릭
2. 확인 모달에서 해제 사유 입력
3. 즉시 휴식 종료 및 활동량 모니터링 재개
...
유저의 아웃(강제 퇴출) 상태를 관리하고 이력을 조회합니다.

![아웃 관리](images/ban_management.svg)

#### 아웃 시스템 개요

**아웃이란?**
- 커뮤니티 규칙 위반 또는 과도한 경고 누적 시 유저를 강제로 제재하는 조치
- 아웃 처리된 유저는 커뮤니티 활동이 제한됨
- 모든 아웃 기록은 영구 보관되며 관리자 로그에 기록됨

**아웃 유형:**
1. **자동 아웃**: 경고 3회 누적 시 시스템이 자동 처리
2. **수동 아웃**: 관리자가 직접 아웃 처리 (스팸, 악의적 행동 등)

#### 통계 카드

- **🚫 현재 아웃 중**: 활성 아웃 상태인 유저 수
- **⚠️ 경고 2회**: 아웃 위험 유저 (3회 시 자동 아웃)
- **📋 총 아웃 기록**: 전체 아웃 처리 이력 건수
- **🔄 해제 건수**: 아웃 해제(언아웃) 처리된 건수

#### 자동 아웃 로직

**⚠️ 중요**: 경고 누적 시스템과 자동 아웃

경고는 1차/2차/3차가 아닌 **누적 횟수**로 관리됩니다:

1. **경고 1회**: 유저에게 경고 DM 발송, 누적 횟수 1회로 기록
2. **경고 2회**: 2번째 경고 DM 발송, 누적 횟수 2회로 기록 (⚠️ 위험 단계)
3. **경고 3회**: 3번째 경고 DM 발송, **즉시 자동 아웃 처리**

**자동 아웃 발생 시:**
- 시스템이 자동으로 `ban_records` 테이블에 기록
- `banned_by`: "system"으로 기록
- `reason`: "자동 아웃: 경고 3회 누적"
- 관리자 로그에 자동 기록
- 유저에게 아웃 통보 DM 발송 (현재 미구현)

**확인 위치:**
- [활동량 모니터링 탭](#31-활동량-모니터링-탭-1)에서 경고 발송 시 "⚠️ 경고 발송 (3/3)" 표시
- 괄호 안 숫자는 발송 후 누적 횟수를 의미
- 3회 발송 시 자동 아웃이 즉시 실행됨

#### 아웃 유저 목록

테이블에서 각 아웃 기록의 상세 정보를 확인할 수 있습니다:

**필터 옵션:**
- **사용자 검색**: 사용자명으로 검색
- **활성 아웃만 표시**: 현재 아웃 중인 유저만 필터링
- **아웃 처리자 필터**: 시스템/특정 관리자별 필터링

**테이블 컬럼:**
- **사용자**: 아웃 처리된 유저 (@마스토돈 아이디)
- **아웃 시점**: 아웃 처리된 날짜와 시간
- **처리자**: 아웃 처리한 관리자 (또는 "system")
- **사유**: 아웃 사유 (예: "스팸 행위", "자동 아웃: 경고 3회 누적")
- **경고 횟수**: 아웃 시점의 누적 경고 횟수
- **상태**: 활성 (🔴) / 해제됨 (✅)
- **작업**: 언아웃/상세보기 버튼

#### 수동 아웃 처리

긴급하게 유저를 아웃 처리해야 할 경우:

![수동 아웃 처리](images/ban_user.svg)

1. 우측 상단 "🚫 유저 아웃 처리" 버튼 클릭
2. 아웃 처리 모달이 열립니다:
   - **대상 유저**: 드롭다운에서 유저 선택
   - **아웃 사유**: 사유 입력 (필수)
     - 예: "스팸 행위", "악의적 언행", "규칙 위반"
   - **증거 스냅샷**: 증거 자료 첨부 (선택사항)
     - 스크린샷 URL, 관련 포스트 링크 등
   - **현재 경고 횟수**: 자동으로 표시됨
3. "🚫 아웃 처리" 버튼 클릭

⚠️ **주의사항**:
- 이미 활성 아웃 상태인 유저는 중복 처리 불가
- 아웃 처리 즉시 유저의 활동이 제한됨 (현재 미구현)
- 모든 아웃 처리는 관리 로그에 영구 기록
- 처리 후 취소 불가 (언아웃만 가능)
...
![언아웃 처리](images/unban_user.svg)

1. 테이블에서 해당 유저 행의 **"언아웃"** 버튼 클릭
2. 언아웃 모달이 열립니다:
   - **기존 아웃 정보**: 아웃 사유, 처리자, 경고 횟수 표시
   - **해제 사유**: 해제 사유 입력 (필수)
     - 예: "경고 후 개선 확인", "오판 확인", "재심사 통과"
   - **경고 횟수 초기화**: 체크박스 선택 옵션
     - ✅ 체크: 누적 경고 횟수를 0으로 초기화
     - ❌ 미체크: 경고 횟수 유지 (다음 경고 시 이어서 계산)
3. "🔄 언아웃 처리" 버튼 클릭

⚠️ **주의사항**:
- 활성 아웃이 없는 유저는 언아웃 처리 불가
- 언아웃 처리 시 `is_active`가 `false`로 변경
- `unbanned_at`, `unbanned_by`, `unban_reason` 필드가 기록됨
- 모든 언아웃 처리는 관리 로그에 기록
- 경고 횟수 초기화 여부는 신중하게 선택

💡 **경고 횟수 초기화 가이드:**
- **초기화 권장**: 오판, 억울한 사유로 아웃된 경우
- **초기화 비권장**: 실제 위반 후 개선이 확인된 경우 (재발 방지)

#### 아웃 이력 조회

각 유저의 과거 아웃 이력을 조회할 수 있습니다:

![아웃 이력](images/ban_history.svg)

1. 테이블에서 유저 행의 **"이력 보기"** 버튼 클릭
2. 이력 모달이 열립니다:
   - **유저 정보**: 마스토돈 아이디, 현재 경고 횟수, 현재 아웃 상태
   - **아웃 이력 목록**:
     - 아웃 시점, 처리자, 사유, 경고 횟수
     - 해제 시점, 해제자, 해제 사유 (해제된 경우)
     - 활성 여부 (🔴 활성 / ✅ 해제됨)
   - **총 아웃 횟수**: 해당 유저의 전체 아웃 처리 건수

💡 **활용 사례:**
- 재발 유저 확인: 과거에도 아웃 이력이 있는지 파악
- 아웃 패턴 분석: 어떤 사유로 주로 아웃되는지 확인
- 관리자 판단 자료: 언아웃 결정 시 과거 이력 참고

#### 관리 로그 연동

모든 아웃 관련 작업은 [관리 로그](#10-관리-로그)에 자동 기록됩니다:

**기록되는 액션:**
- `action_type`: "manual_ban" (수동 아웃)
- `action_type`: "auto_ban" (자동 아웃)
- `action_type`: "unban" (언아웃)
- `target_user`: 대상 유저 ID
- `admin_name`: 처리한 관리자 (자동 아웃은 "system")
- `details`: 상세 사유 및 경고 횟수

**로그 조회 예시:**
1. 상단 메뉴 → 📋 로그
2. 액션 타입 필터: "아웃 관리"만 체크
3. 특정 유저의 아웃 이력 확인: 검색창에 유저 ID 입력

#### 아웃 vs 휴식

**아웃과 휴식의 차이:**

| 구분 | 아웃 (Ban) | 휴식 (Vacation) |
|------|------------|-----------------|
| 목적 | 제재 조치 | 활동 일시 중단 |
| 처리 방식 | 관리자 또는 시스템 | 유저 신청 또는 관리자 등록 |
| 활동 제한 | 제한됨 (현재 미구현) | 제한 없음 (단, 체크 제외) |
| 재화 지급 | 중단 | 중단 |
| 활동량 체크 | 제외됨 | 제외됨 |
| 경고 발송 | 불가 | 불가 |
| 해제 방법 | 관리자 언아웃 | 기간 만료 또는 강제 해제 |
| 기록 | 영구 보관 | 영구 보관 |

⚠️ **주의**: 아웃은 제재 조치이므로 휴식과 달리 유저가 직접 신청할 수 없습니다.

#### FAQ

**Q1. 경고 3회 누적 시 바로 아웃되나요?**
- 네, 경고 3회 발송 시 즉시 자동 아웃 처리됩니다.
- 자동 아웃은 `UserService.add_warning()` 메서드에서 실행됩니다.

**Q2. 자동 아웃된 유저를 언아웃하려면?**
- 일반 언아웃과 동일한 절차입니다.
- 단, 경고 횟수 초기화 여부를 신중하게 결정하세요.

**Q3. 아웃 처리된 유저가 활동할 수 있나요?**
- 현재는 기술적 제한이 미구현 상태입니다.
- 봇 명령어 사용 제한 기능은 TODO 상태입니다.
- 관리자가 수동으로 모니터링해야 합니다.

**Q4. 아웃 기록을 삭제할 수 있나요?**
- 아니요, 아웃 기록은 영구 보관됩니다.
- 단, 언아웃 처리로 활성 상태를 해제할 수 있습니다.

**Q5. 경고 횟수만 초기화하고 싶어요**
- 유저 관리 → 재화 관리 탭에서 직접 DB 수정이 필요합니다.
- 또는 개발자에게 SQL 실행 요청하세요.
- 향후 UI에서 지원 예정입니다.
...
2. "⚠️ 경고 발송" 탭 선택
3. 경고 대상 유저 목록에서:
   - 누적 경고 횟수별 현황 카드로 전체 상황 파악
   - 테이블에서 대상 유저 확인 (누적 경고 횟수, 답글 수, 최근 경고)
4. 발송 방법 선택:
...
- 누적 경고 횟수에 따라 자동으로 적절한 템플릿 메시지가 발송됩니다
...
대시보드 통계 ("전체 유저", "총 재화" 등)는 이제 **시스템 계정을 자동으로 제외**합니다.
...
1. **마스토돈 웹 UI**에서 역할 관리
   - `https://[서버]/admin/roles` 접속
   - 원하는 역할 생성 (예: "봇", "시스템")
   - 해당 계정에 역할 할당

2. **자동 동기화**
   - OAuth 로그인 시 자동으로 역할 정보 가져옴
   - 수동 관리 불필요

3. **확인**
   - 대시보드에서 통계가 정확히 표시되는지 확인

### 자세한 설명

역할 설정 방법은 `docs/MASTODON_ROLES.md` 참조
