...
- **💰 총 유통 재화**: 모든 유저의 보유 재화 합계 (커뮤니티 경제 규모)
- **📊 평균 보유**: 유저당 평균 재화 보유량
...
- **표시명**: 유저의 표시 이름
- **현재 잔액**: 현재 보유 중인 재화
- **누적 획득**: 가입 이후 획득한 총 재화
- **누적 사용**: 가입 이후 사용한 총 재화
- **작업**: "상세/조정" 버튼 → 거래 내역 상세 페이지로 이동

#### 유저별 거래 내역 상세 (토스 스타일)

![유저 재화 상세](images/user_wallet.svg)

사용자명을 클릭하거나 "상세/조정" 버튼을 누르면 해당 유저의 상세 거래 내역 페이지로 이동합니다.
...
**2. 거래 내역 테이블 (토스 통장 스타일)**
- **거래일시**: 거래가 발생한 날짜와 시간
- **적요(사유)**: 거래 사유 및 카테고리 (예: [이벤트 보상] 출석 이벤트 참여)
- **출금**: 차감된 금액 (빨간색)
- **입금**: 지급된 금액 (초록색)
- **잔액**: 거래 후 잔액
- **담당**: 거래를 처리한 관리자 또는 시스템
- **작업**: 수정/삭제 버튼

**3. 신규 거래 추가**

![신규 거래 추가](images/user_wallet_add_transaction.svg)

"+ 신규 거래" 버튼을 클릭하면 모달이 열립니다:
...
⚠️ **주의사항**:
- 거래 추가 시 잔액이 자동으로 계산됩니다
- 모든 거래는 관리 로그에 영구 기록됩니다
- 사유는 필수 항목입니다

**4. 거래 수정**

![거래 수정](images/user_wallet_edit_transaction.svg)

각 거래 행의 "수정" 버튼을 클릭하면 수정 모달이 열립니다:

- 기존 거래 정보가 채워진 상태로 표시
- 금액, 사유, 카테고리, 세부 설명 수정 가능
- 수정 시 잔액이 자동으로 재계산됨

⚠️ **주의**: 거래 수정 시 잔액이 재계산되므로 신중하게 수정하세요.
