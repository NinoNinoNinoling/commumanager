...
시스템은 **4가지 관점**에서 커뮤니티 건강도를 분석합니다:

#### 🎯 고립 위험
- **기준**: 대화 상대가 접속 중인 인원의 절반 미만
- **의미**: 소수와만 교류하여 커뮤니티 통합 저해
- **색상**: 보라색 (#6f42c1)

#### ⚖️ 편향 위험
- **기준**: 특정 1인과의 대화가 전체의 10% 이상
- **의미**: 특정인에게 과도하게 집중된 교류 패턴
- **색상**: 주황색 (#fd7e14)

#### 🚫 회피 패턴
- **기준**: 활동은 활발하지만 특정 주요 멤버를 회피
- **의미**: 커뮤니티 주요 구성원과의 교류 부족
- **색상**: 빨간색 (#dc3545)
...
💡 **Tip:** 한 유저가 여러 위험 유형에 동시에 해당할 수 있습니다. 예를 들어 "편향 위험"과 "답글 미달"을 모두 가질 수 있습니다.

💡 **참고:** 고립 위험 기준은 현재 접속 중인 인원에 따라 동적으로 변합니다. 예를 들어 10명이 접속 중이라면 5명 이상과 대화해야 정상, 4명 이하면 고립 위험으로 감지됩니다.
...
- **⚠️ 활동량 모니터링**: 5가지 관점에서 유저 활동 건강도 분석
- **🏖️ 휴식 현황**: 유저가 신청한 휴식 기간 조회 및 강제 해제
...
시스템은 **5가지 위험 유형**으로 유저를 분석합니다. 체크박스로 원하는 유형만 선택하여 조회할 수 있습니다:

**🎯 고립 위험**
- **기준**: 48시간 내 3명 이하와만 대화
- **설명**: 소수와만 교류하는 유저
- **예시**: userA가 userB, userC 2명하고만 대화

**⚖️ 편향 위험**
- **기준**: 특정 1인과의 대화가 전체의 20% 이상
- **설명**: 한 유저에게 과도하게 집중된 교류
- **예시**: userA의 전체 대화 중 userB와의 대화가 25%
...
- **예시 1**: `편향` + `미달` → "@userB와 25% / 4개 답글"
- **예시 2**: `고립` + `미달` + `비활동` → "3명과만 대화 / 2개 답글 / 2회 접속"
...
💡 **관리자 역할**: 관리자는 등록하는 것이 아니라 **조회 및 강제 해제**만 가능합니다.

#### 휴식 목록 조회

테이블에서 각 휴식의 상세 정보를 확인할 수 있습니다:

- **사용자**: 휴식 신청한 유저 (본인이 직접 신청)
- **시작일**: 휴식 시작 날짜
- **종료일**: 휴식 종료 예정 날짜
- **기간**: 총 N일
- **사유**: 유저가 입력한 사유 (선택사항)
...
⚠️ **개별 유저 휴식**과 구분:
- 개별 유저 휴식: 특정 유저만 활동량 체크 제외 → [3.2절 참조](#32-휴식-관리-탭-2)
